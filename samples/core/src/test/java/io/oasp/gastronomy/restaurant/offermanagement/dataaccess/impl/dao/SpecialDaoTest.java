...
public class SpecialDaoTest extends ComponentTest {
  ...
  /*
   * @Test public void testNotFindingBestActiveSpecialWhenNoSpecialExists() {
   *
   * // given SpecialSearchCriteriaTo searchCriteria = new SpecialSearchCriteriaTo();
   * searchCriteria.setOfferNumber(102L); searchCriteria.setDateOfCheckingOffers(LocalDateTime.of(2018, 2, 2, 2, 0));
   *
   * // when Money bestSpecialPrice = this.specialDao.findBestActiveSpecial(searchCriteria);
   *
   * // then assertThat(bestSpecialPrice).isNull(); }
   */

  /*
   * @Test public void testFindingBestActiveSpecialWhenTwoDifferentExist() {
   *
   * // given LocalDateTime currentDateTime = LocalDateTime.of(2018, 2, 2, 12, 0); OfferEntity offer = prepareOffer();
   * SpecialEntity specialWithWorsePrice = prepareSpecialWithOffer(offer); specialWithWorsePrice.setSpecialPrice(new
   * Money(11)); setSpecialActiveForCurrentDateTime(specialWithWorsePrice, currentDateTime); SpecialEntity
   * specialWithBetterPrice = prepareSpecialWithOffer(offer); specialWithBetterPrice.setSpecialPrice(new Money(9));
   ...
   * // when Money bestSpecialPrice = this.specialDao.findBestActiveSpecial(searchCriteria);
   *
   * // then assertThat(bestSpecialPrice).isEqualTo(specialWithBetterPrice.getSpecialPrice()); }
   */
  /*
   * @Test public void testFindingBestActiveSpecialWhenTwoEqualtExist() {
   *
   * // given LocalDateTime currentDateTime = LocalDateTime.of(2018, 2, 2, 12, 0); OfferEntity offer = prepareOffer();
   * SpecialEntity special1 = prepareSpecialWithOffer(offer); setSpecialActiveForCurrentDateTime(special1,
   * currentDateTime); SpecialEntity special2 = prepareSpecialWithOffer(offer);
   * special2.setName("Special with same price"); setSpecialActiveForCurrentDateTime(special2, currentDateTime);
   ...
   * // when Money bestSpecialPrice = this.specialDao.findBestActiveSpecial(searchCriteria);
   *
   * // then assertThat(bestSpecialPrice).isEqualTo(special2.getSpecialPrice()); }
   */
